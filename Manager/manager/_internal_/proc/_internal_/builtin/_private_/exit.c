...
/*   Updated: 2025/07/24 10:16:53 by nduvoid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline struct s_args_exit	_parse(
	const char **args
)
{
...
__attribute__((always_inline, used)) static inline char	_help(void)
{
	...
	exit_program(EXIT_FAILURE, DEFAULT_EXIT_MESSAGE);	// @todo: get this in the config
	return (EXIT_FAILURE);
}
...
)
{
	...
	exit_program(exit_code, DEFAULT_EXIT_MESSAGE);	// @todo: get this in the config
	return (EXIT_FAILURE);
}
 ...
 * @version 1.0
*/
__attribute__((used, visibility("hidden"))) char	bltin_exit(
	const char **args,
	const int fd_in,
	const int fd_out
)
{
	...
	if (parsed.help)
		...
		exit_program(parsed.exit_code, DEFAULT_EXIT_MESSAGE);	// @todo: get this in the config
	return (EXIT_SUCCESS);
}
