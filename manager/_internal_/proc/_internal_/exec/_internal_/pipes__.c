...
/*   Updated: 2025/07/24 09:11:52 by nduvoid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
inline int	_exec_piping__(
	int pipe_fd[2],
	int *const restrict out_fd
)
{
	if (unexpect(pipe(pipe_fd) < 0))
		return (perror("full_exec(): pipe failed"), -1);
	else if (expect(out_fd != NULL))
		*out_fd = pipe_fd[1];
	fdm_register(pipe_fd[0]);	// @todo: replace by manager call
